...
if (isset($_FILES["avatar"]["name"]) && $_FILES["avatar"]["tmp_name"] != ""){
	...
	if($width < 10 || $height < 10){
		header("location: ../user/message.php?msg=ERROR: That image has no dimensions");
        exit();	
	}
	$db_file_name = rand(100000000000,999999999999).".".$fileExt;
	if($fileSize > 2048576) {
		header("location: ../user/message.php?msg=ERROR: Your image file was larger than 2mb");
		exit();	
	...
	$avatar = $row[0];
	if($avatar != ""){
		$picurl = "../_Users/$log_username/$avatar"; 
	...
	$sql = "UPDATE users SET avatar='$db_file_name' WHERE username='$log_username'";
	$query = mysqli_query($db_conx, $sql);
	mysqli_close($db_conx);
...
if (isset($_FILES["photo"]["name"]) && isset($_POST["gallery"])){
	...
	$row = mysqli_fetch_row($query);
	if($row[0] > 49){
		header("location: ../user/message.php?msg=The demo version of bleak allows only 50 pictures total");
        exit();	
	}
	$gallery = preg_replace('#[^a-z 0-9,]#i', '', $_POST["gallery"]);
	...
	if($fileSize > 3048576) {
		header("location: ../user/message.php?msg=ERROR: Your image file was larger than 3mb");
		exit();	
	} else if (!preg_match("/\.(gif|jpg|png)$/i", $fileName) ) {
		header("location: ../user/message.php?msg=ERROR: Your image file was not jpg, gif or png type");
		exit();
	...
	$query = mysqli_query($db_conx, $sql);
	mysqli_close($db_conx);
	header("location: ../profile/photos.php?u=$log_username");
	exit();
}
?><?php 
if (isset($_POST["delete"]) && $_POST["id"] != ""){
	$id = preg_replace('#[^0-9]#', '', $_POST["id"]);
	$query = mysqli_query($db_conx, "SELECT user, filename FROM photos WHERE id='$id'");
	$row = mysqli_fetch_row($query);
    $user = $row[0];
	$filename = $row[1];
	if($user == $log_username){
		$picurl = "../_Users/$log_username/$filename"; 
