...
if (isset($_POST['type']) && isset($_POST['user'])){
	...
	if($_POST['type'] == "friend"){
	    ...
	    if($friend_count[0] > 199){
            mysqli_close($db_conx);
	        echo "Sorry, $user currently has the maximum number of friends, and cannot accept more.";
	        exit();
        } else if($blockcount1[0] > 0){
            mysqli_close($db_conx);
	        echo "Oops! $user has you blocked, we cannot proceed.";
	        exit();
        } else if($blockcount2[0] > 0){
            mysqli_close($db_conx);
	        echo "Wait! ..You must first unblock $user. Remember you blocked this user?";
	        exit();
        } else if ($row_count1[0] > 0 || $row_count2[0] > 0) {
		    mysqli_close($db_conx);
	        echo "Hey! You are already friends with $user.";
	        exit();
	    } else if ($row_count3[0] > 0) {
		    mysqli_close($db_conx);
	        echo "Hmmm .. You have a pending friend request already sent to $user.";
	        exit();
	    } else if ($row_count4[0] > 0) {
		    mysqli_close($db_conx);
	        echo "$user has already requested to be friends with you. Check your friend requests.";
	        exit();
	    } else {
...
if (isset($_POST['action']) && isset($_POST['reqid']) && isset($_POST['user1'])){
	...
	if($_POST['action'] == "accept"){
	    ...
	    if ($row_count1[0] > 0 || $row_count2[0] > 0) {
		    mysqli_close($db_conx);
	        echo "Nope! .. You are already friends with $user.";
	        exit();
	    } else {
		...
		mysqli_query($db_conx, "DELETE FROM friends WHERE id='$reqid' AND user1='$user' AND user2='$log_username' AND accepted='0'");
		mysqli_close($db_conx);
		echo "reject_ok";
		exit();
	}
