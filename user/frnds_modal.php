...
if($friend_count < 1 && $u == $log_username){
...
} else if($u != $log_username && $friend_count < 1){
	$friendsList = '<span style="color: rgb(45, 137, 180);margin-left:38%;"> '.$u.' has no friends yet </span>';
	...
	while($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		$friendsList .= '<span class="b" href="../profile/?u='.$friend_username.'">';
		$friendsList .= '<div class="qf">';
		$friendsList .= '<span class="qj">';
		$friendsList .= '<a href="../profile/?u='.$friend_username.'">';
		...
		$friendsList .= '<strong><a href="../profile/?u='.$friend_username.'">'.$friend_username.'</a></strong>';
		$friendsList .= '<div class="aof">'.$friendstatus.'</div>';
		$friendsList .= '</div></div></span>';
		
		$total_friends = '<b>You have a total of '.$friend_count.' friends.</b>';	
		if($u != $log_username){
		$total_friends = '<b>'.$u.' has a total of '.$friend_count.' friends.</b>';
		$hide_search = 'style="display:none;"';
		}
	}
}
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="d">
        ...
        <h4 class=""><?php echo $person ?> Friends</h4>
      </div>
